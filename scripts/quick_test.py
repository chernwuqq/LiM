#!/usr/bin/env python3
"""
Quick test of tl2cgen C model vs Python
"""
import pandas as pd
import numpy as np
...
print("=" * 60)
print("Python vs tl2cgen C Model")
print("=" * 60)
...
for i in range(10):
    ...
    c_class = le.classes_[c_pred_idx] if c_pred_idx is not None else "ERR"
    true_class = le.classes_[y_test[i]]
    
    match = "Y" if py_class == c_class else "N"
    print(f"{i+1:<3} {true_class:<12} {py_class:<12} {c_class:<12} {match:<6}")

print("-" * 50)
print("Note: C model compiled from tl2cgen")